...
### Includes Code to handle breaking changes for Release 4.5.1 in the get_initial_item_state function ########
##############################################################################################################
##############################################################################################################
...
def get_initial_item_state(layout_element:str,item_id:int):
    ...
    if layout_element not in sig_file_config.keys():
        ...
        ###########################################################################################################
        ### Handle breaking change for Release 4.5.1 where secondary distants are now allocated a signal ID #######
        ### of Home Signal ID + 1000 (rather than Home Signal ID + 100). Therefore if we can't find an entry ######
        ### for a signal with an ID over 1000 then assume we are olding a layout file generated by a previous #####
        ### version of the application and modify the ID to match the old numbering scheme (Home ID + 100) ########
        ### If we still cant find an entry for the signal (see later in the code) then this is a valid warning ####
        ###########################################################################################################
        if layout_element in layout_state.keys() and str(item_id) not in layout_state[layout_element].keys() and item_id > 1000:
            item_id = item_id - 900
        ###########################################################################################################
        ### End of handle breaking changes for Release 4.5.1 (secondary distant ID now Home ID + 1000 #############
        ###########################################################################################################
