...
def check_conflicting_signals(route_object:dict, route_tooltip:str, route_viable:bool, subsidaries:bool=False):
    ...
    for signal_id in route_object[signals_on_route_dict_key]:
        ...
        if signal_route is None or not signal_object[signal_routes_dict_key][signal_route.value-1]:
            ...
            # the signal would be locked by a block instrument ahead, its subsidiary or occupied track
            # sections (on the theoretical route ahead)
            if not subsidaries:
                # Check locking with Block Instruments
                ...
                # Check Locking of Subsidiary with co-located Signal
                if library.signal_clear(int(signal_id)):
                    route_viable = False
                    message = "\n"+sig_type+str(signal_id)+" would be locked by signal "+str(signal_id)
                    route_tooltip = route_tooltip + message                    
...
def check_conflicting_points(route_object:dict, route_tooltip:str, route_viable:bool):
    ...
    for str_point_id in route_object["pointsonroute"]:
...
#------------------------------------------------------------------------------------
# Function to initialise all routes after layout load or switching between edit/run modes.
#
# If we are in edit mode then any route buttons that are current 'set' will be 'unset' and
# the colour of all points/lines reverted to their default colours. Note that we reset all
# signals and subsidaries along the route but leave all points and DCC switches in their
# current states rather that fully 'clearing down' the route (unlike the 'reset' use case).
#
...
# and lines will have been recreated with their default colours, so we need to change the colour
# for any routes wich are enabled. Note we clear down all route highlighting in edit mode
#------------------------------------------------------------------------------------
...
def initialise_all_schematic_routes():
    ...
    # We also clear down all signals along the route (otherwise NX routes can get into a funny state)
        ...
        for str_route_button_id in objects.route_index:
            if library.button_state(int(str_route_button_id)):
                route_definition_index = library.get_button_data(int(str_route_button_id))["route"]
                # If a 'one-click' button or an Entry button then there will be a route definition
                # We can therefore clear down all signals and subsidaries within the route definition
                if route_definition_index is not None:
                    route_object = objects.schematic_objects[objects.route(int(str_route_button_id))]
                    route_definition = route_object["routedefinitions"][route_definition_index]
                    for str_signal_id in route_definition["signalsonroute"]:
                        automatic_signal = objects.schematic_objects[objects.signal(str_signal_id)]["fullyautomatic"]
...
def check_routes_valid_after_point_change(point_id:int, route_button_id:int):
    for str_route_button_id in objects.route_index:
        ...
        if route_definition_index is not None and not is_setup_in_progress(int(str_route_button_id)):
            ...
            if ( int(str_route_button_id) != route_button_id and library.button_state(int(str_route_button_id))
                     and str(point_id) in route_definition["pointsonroute"] ):
    ...
    if switch_type == library.button_type.switched.value:
        for str_route_button_id in objects.route_index:
            ...
            if route_definition_index is not None and not is_setup_in_progress(int(str_route_button_id)):
                ...
                # Reset the route if any points on the route have been unlocked or changed
                if ( int(str_route_button_id) != route_button_id and library.button_state(int(str_route_button_id))
                        and str(switch_id) in route_definition["switchesonroute"] and route_object["resetonswitchchanges"] ):
...
def complete_route_setup(route_button_id:int, dont_enable_disable_schematic_routes:bool=False):
    ...
    if route_definition_index is not None:
        ...
        for str_point_id in route_definition["pointsonroute"]:
        ...
        for str_switch_id in route_definition["switchesonroute"]:
...
def route_button_selected_callback(route_button_id:int):
    ...
    if not route_object["entrybutton"] and not route_object["exitbutton"]:
        ...
        schedule_tasks_to_setup_schematic_route(route_button_id,
        ...
        for index, route_definition in enumerate(entry_route_object["routedefinitions"]):
            if route_definition["exitbutton"] == route_button_id:
                ...
                unhighlight_possible_routes(activated_entry_button_id)
                schedule_tasks_to_setup_schematic_route(activated_entry_button_id,
                        route_definition_index=index, delay=route_object["switchdelay"])
...
def route_button_deselected_callback(route_button_id:int):
    ...
    if activated_entry_button_id == route_button_id:
        ...
        if route_button_data["entrybutton"] > 0 and library.button_state(route_button_data["entrybutton"]):
            ...
            logging.debug("RUN ROUTES - Leaving button "+str(route_button_id)+" deselected as this is not an part of an active route")
        logging.debug("RUN ROUTES - Clearing down current NX route setup sequence")
        unhighlight_possible_routes(activated_entry_button_id)
        activated_entry_button_id = 0
    # If there is an activated ENTRY button (where an exit button has not yet been selected) and this
    # button is an EXIT button then the user could be setting up a route TO this EXIT Button). If the
    # button is not already an EXIT button on an active route, then we toggle the button to ON and make
    ...
    # toggle the button back to ON and we clear down the current selection sequence.
        ...
        if route_object["exitbutton"]:
            ...
            library.toggle_button(route_button_id)
            reset_activated_entry_button = True
        if reset_activated_entry_button:
            # Toggle the activated_entry_button OFF unless it is the entry or exit button for an active route
            if (library.button_state(activated_entry_button_id) and activated_entry_button_data["entrybutton"] == 0):
                logging.debug("RUN ROUTES - Deselecting Activated ENTRY Button: "+str(activated_entry_button_id))
                library.toggle_button(activated_entry_button_id)
            else:
                logging.debug("RUN ROUTES - Leaving Activated ENTRY Button "+str(activated_entry_button_id)+
                                            " selected as this is part of an active route")
            logging.debug("RUN ROUTES - Clearing down current NX route setup sequence")
            unhighlight_possible_routes(activated_entry_button_id)
            activated_entry_button_id = 0
    ...
    # If the button has an active route set up then it is an ENTRY button
        ...
        if not route_button_data["entrybutton"] and not route_button_data["exitbutton"]:
            ...
            logging.debug("RUN ROUTES - ENTRY Button "+str(route_button_id)+" has been deselected")
        ...
        if route_button_data["exitbutton"] > 0:
            exit_button_data = library.get_button_data(route_button_data["exitbutton"])
            if library.button_state(route_button_data["exitbutton"]) and exit_button_data["route"] is None:
                logging.debug("RUN ROUTES - No active route beyond associated Exit Button "
                         +" so deselecting exit button "+str(route_button_data["exitbutton"]))
                # Lock the Exit Button during route clear-down 
                library.toggle_button(route_button_data["exitbutton"])
                library.lock_button(route_button_data["exitbutton"])
                library.disable_button(route_button_data["exitbutton"], tooltip="Route clear down in progress")
        if route_button_data["entrybutton"] > 0:
            entry_button_data = library.get_button_data(route_button_data["entrybutton"])
            if library.button_state(route_button_data["entrybutton"]) and entry_button_data["route"] is not None:
                logging.debug("RUN ROUTES - Reselecting Button "+str(route_button_id)+" as this is also an EXIT button "
                              +"for an active route from Button " +str(route_button_data["entrybutton"]))
                library.toggle_button(route_button_id)
        # Schedule the tasks to clear down the route 
        logging.info("RUN ROUTES - Initiating clear-down of route from Button "+str(route_button_id))
        schedule_tasks_to_clear_down_schematic_route(route_button_id,
                    route_button_data["route"], switch_delay=route_object["switchdelay"])
    else:
        # Route must be none and activated_entry button must be none- this could be part of a new selection sequence
        logging.info("RUN ROUTES - Button "+str(route_button_id)+" has been de-selected")
        logging.debug("RUN ROUTES - Reselecting Button "+str(route_button_id)+" as this is part of an active route")
        library.toggle_button(route_button_id)
        logging.debug("RUN ROUTES - Checking to see if Button "+str(route_button_id)+
                                    " is part of a new NX route setup sequence")
...
def schedule_tasks_to_clear_down_schematic_route(route_button_id:int, route_definition_index:int, switch_delay:int):
    # Find the applicable route definition.
    route_object = objects.schematic_objects[objects.route(route_button_id)]
    route_definition = route_object["routedefinitions"][route_definition_index]
    ...
    if route_object["resetpoints"]:
        delay = schedule_tasks_to_reset_points(route_definition["pointsonroute"],
                                            switch_delay, route_button_id, delay=delay)
    # Schedule tasks to reset all the DCC Switches back to "OFF"
    if route_object["resetswitches"]:
        delay = schedule_tasks_to_reset_switches(route_definition["switchesonroute"],
                                            switch_delay, route_button_id, delay=delay)
...
def reset_remaining_routes():
    for route_button_id in objects.route_index:
