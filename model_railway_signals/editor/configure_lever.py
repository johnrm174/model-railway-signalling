...
class signal_configuration(Tk.LabelFrame):
    def __init__(self, parent_window):
        ...
        self.signalid = common.int_item_id_entry_box(self.frame, tool_tip="Enter the ID of the signal to switch",
                                    callback=self.signal_id_updated, exists_function=signals.signal_exists)
...
class point_configuration(Tk.LabelFrame):
    def __init__(self, parent_window):
        ...
        self.pointid = common.int_item_id_entry_box(self.frame, tool_tip="Enter the ID of the point to switch",
                                    callback=self.point_id_updated, exists_function=points.point_exists)
...
#------------------------------------------------------------------------------------
    ...
    def save_state(self, close_window:bool):
        ...
        # been validated on entry, but changes to other objects may have been made since then
            ...
            lever_type = levers.lever_type.spare.value
            if point_id > 0:
                if point_lever_subtype == 1: lever_type = levers.lever_type.point.value
                elif point_lever_subtype == 2: lever_type = levers.lever_type.pointfpl.value
                elif point_lever_subtype == 3: lever_type = levers.lever_type.pointwithfpl.value
            elif signal_id > 0:
                if signal_lever_subtype == 1:
                    signal_object = objects.schematic_objects[objects.signal(signal_id)]
                    if signal_object["itemsubtype"] == signals.signal_subtype.distant.value:
                        lever_type = levers.lever_type.distantsignal.value
                    else:
                        lever_type = levers.lever_type.stopsignal.value
                if signal_lever_subtype == 2: lever_type = levers.lever_type.stopsignal.value
                if signal_lever_subtype == 3: lever_type = levers.lever_type.distantsignal.value
