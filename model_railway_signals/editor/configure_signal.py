...
#    signals.signal_type - The sygnal type
#    signals.signal_subtype - colour light signal sub-type
#    signals.semaphore_subtype - semaphore signal sub-type
#
...
def has_subsidary(signal):
    return ( ( signal.config.sigtype.get_value() == signals.signal_type.semaphore.value and
             ...
             (signal.config.sigtype.get_value() == signals.signal_type.colour_light.value and
...
def get_sub_routes(signal):
    # Get the route selections from the appropriate UI element
    if ( signal.config.sigtype.get_value() == signals.signal_type.ground_position.value or
          signal.config.sigtype.get_value() == signals.signal_type.ground_disc.value):
        routes = [False, False, False, False, False]
    elif signal.config.sigtype.get_value() == signals.signal_type.colour_light.value:
...
def update_tab1_signal_aspect_selections(signal):
    if signal.config.sigtype.get_value() == signals.signal_type.colour_light.value:
        if signal.config.subtype.get_value() == signals.signal_subtype.home.value:
        ...
        elif signal.config.subtype.get_value() == signals.signal_subtype.distant.value:
            ...
            signal.config.aspects.fdylw.disable_addresses()
        elif signal.config.subtype.get_value() == signals.signal_subtype.red_ylw.value:
            signal.config.aspects.red.enable_addresses()
            ...
            signal.config.aspects.fylw.disable_addresses()
            signal.config.aspects.fdylw.disable_addresses()
        elif signal.config.subtype.get_value() == signals.signal_subtype.three_aspect.value:
            signal.config.aspects.red.enable_addresses()
            signal.config.aspects.grn.enable_addresses()
            signal.config.aspects.ylw.enable_addresses()
            signal.config.aspects.dylw.disable_addresses()
            signal.config.aspects.fylw.enable_addresses()
            signal.config.aspects.fdylw.disable_addresses()
        elif signal.config.subtype.get_value() == signals.signal_subtype.four_aspect.value:
            signal.config.aspects.red.enable_addresses()
            signal.config.aspects.grn.enable_addresses()
            signal.config.aspects.ylw.enable_addresses()
    ...
    if ( signal.config.sigtype.get_value() == signals.signal_type.colour_light.value and
         signal.config.subtype.get_value() != signals.signal_subtype.distant.value ):
...
def update_tab1_route_selection_elements(signal):
    if signal.config.sigtype.get_value() == signals.signal_type.colour_light.value:
        ...
        if signal.config.subtype.get_value() == signals.signal_subtype.distant.value:
        ...
        if signal.config.subtype.get_value() == signals.semaphore_subtype.distant.value:
...
def update_tab2_available_signal_routes(signal):
    ...
    if signal.config.sigtype.get_value() == signals.signal_type.semaphore.value:
...
def update_tab2_interlock_ahead_selection(signal):
    if ( ( signal.config.sigtype.get_value() == signals.signal_type.semaphore.value and
           signal.config.subtype.get_value() == signals.semaphore_subtype.distant.value) or
         ( signal.config.sigtype.get_value() == signals.signal_type.colour_light.value and
           signal.config.subtype.get_value() == signals.signal_subtype.distant.value) or
         ( signal.config.sigtype.get_value() == signals.signal_type.semaphore.value and
           signal.config.subtype.get_value() == signals.semaphore_subtype.home.value and
...
def update_tab3_signal_ui_elements(signal):
    ...
    # Only pack those elements relevant to the signal type and route type
    if ( signal.config.sigtype.get_value() == signals.signal_type.colour_light.value or
         signal.config.sigtype.get_value() == signals.signal_type.semaphore.value ):
        signal.automation.timed_signal.frame.pack(padx=2, pady=2, fill='x')
    rel_on_red = ( ( signal.config.sigtype.get_value() == signals.signal_type.colour_light.value and
                     signal.config.subtype.get_value() != signals.signal_subtype.distant.value) or
                   ( signal.config.sigtype.get_value() == signals.signal_type.semaphore.value and
                     signal.config.subtype.get_value() != signals.semaphore_subtype.distant.value ) )
    rel_on_yel = ( signal.config.sigtype.get_value() == signals.signal_type.colour_light.value and
                   signal.config.subtype.get_value() != signals.signal_subtype.home.value and
                   signal.config.subtype.get_value() != signals.signal_subtype.distant.value and
                   signal.config.subtype.get_value() != signals.signal_subtype.red_ylw.value )
...
def update_tab3_general_settings_selections(signal):
    ...
    if ( signal.config.sigtype.get_value() == signals.signal_type.semaphore.value  or
         signal.config.sigtype.get_value() == signals.signal_type.colour_light.value):
    ...
    if ( ( signal.config.sigtype.get_value() == signals.signal_type.colour_light.value and
           signal.config.subtype.get_value() != signals.signal_subtype.home.value) or
         ( signal.config.sigtype.get_value() == signals.signal_type.semaphore.value and
           signal.config.subtype.get_value() != signals.semaphore_subtype.home.value ) or
         ( signal.config.sigtype.get_value() == signals.signal_type.semaphore.value and
           has_secondary_distant(signal) ) ):
...
def update_tab3_timed_signal_selections(signal):
    ...
    timed_signal = (signal.config.sigtype.get_value() == signals.signal_type.semaphore.value or
                    signal.config.sigtype.get_value() == signals.signal_type.colour_light.value )
...
def update_tab3_approach_control_selections(signal):
    ...
    rel_on_yel = ( signal.config.sigtype.get_value() == signals.signal_type.colour_light.value and
                   ...
                   signal.config.subtype.get_value() != signals.signal_subtype.distant.value and
                   signal.config.subtype.get_value() != signals.signal_subtype.red_ylw.value )
    rel_on_sig = ( ( signal.config.sigtype.get_value() == signals.signal_type.colour_light.value and
                     signal.config.subtype.get_value() == signals.signal_subtype.home.value )or
                   ( signal.config.sigtype.get_value() == signals.signal_type.semaphore.value and
                     signal.config.subtype.get_value() == signals.semaphore_subtype.home.value ) )
...
#------------------------------------------------------------------------------------
    ...
    def load_state(self):
        ...
        if self.object_id not in objects.schematic_objects.keys():
            ...
            if objects.schematic_objects[self.object_id]["itemtype"] == signals.signal_type.colour_light.value:
            ...
            elif objects.schematic_objects[self.object_id]["itemtype"] == signals.signal_type.semaphore.value:
                if objects.schematic_objects[self.object_id]["theatreroute"]:
                    self.config.routetype.set_value(3)
