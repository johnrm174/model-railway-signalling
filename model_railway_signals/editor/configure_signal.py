...
def update_tab2_available_signal_routes(signal):
    ...
    if signal.config.routetype.get_value() == 2:
        ...
        if signal.config.feathers.lh2.get_feather(): signal.locking.sig.lh2.enable()
        else: signal.locking.sig.lh2.disable()
        if signal.config.feathers.rh1.get_feather(): signal.locking.sig.rh1.enable()
        else: signal.locking.sig.rh1.disable()
        if signal.config.feathers.rh2.get_feather(): signal.locking.sig.rh2.enable()
        else: signal.locking.sig.rh2.disable() 
    # Theatre Indicator
    elif signal.config.routetype.get_value() == 3:
        if signal.config.theatre.lh1.get_theatre()[0] !="": signal.locking.sig.lh1.enable()
        else: signal.locking.sig.lh1.disable()
        if signal.config.theatre.lh2.get_theatre()[0] !="": signal.locking.sig.lh2.enable()
        else: signal.locking.sig.lh2.disable()
        if signal.config.theatre.rh1.get_theatre()[0] !="": signal.locking.sig.rh1.enable()
        else: signal.locking.sig.rh1.disable()
