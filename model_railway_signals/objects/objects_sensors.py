...
#    library.create_track_sensor(id) - Create the library object
#    library.delete_track_sensor(id) - Delete the library object
#    library.track_sensor_exists - to find out if the specified Item ID already exists
...
def redraw_track_sensor_object(object_id):
    ...
    if gpio_sensor != "": library.update_gpio_sensor_callback(gpio_sensor, sensor_passed=item_id)
    return()
...
def create_track_sensor(xpos:int, ypos:int):
    ...
    item_id = objects_common.new_item_id(exists_function=library.track_sensor_exists)
...
def paste_track_sensor(object_to_paste, deltax:int, deltay:int):
    ...
    new_id = objects_common.new_item_id(exists_function=library.track_sensor_exists)
