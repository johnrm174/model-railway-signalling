...
def switch_button(switch_id,button_id):
    ...
    sections.override_signals_based_on_track_occupancy() # for track occupancy changes
    schematic.update_track_schematic(canvas) # for track power section changes
...
def point_callback_function(point_id,callback_type):
    ...
    sections.override_signals_based_on_track_occupancy() # for route changes
    sections.update_track_power_section_switches() # for when automatic switching is enabled
    interlocking.process_interlocking_east()
    interlocking.process_interlocking_west()
    schematic.update_track_schematic(canvas)
    schematic.refresh_signal_aspects()
    return()
...
def signal_callback_function(sig_id,callback_type):
    ...
    if callback_type == signals.sig_callback_type.sig_passed:
        sections.update_track_occupancy(sig_id)
    sections.override_signals_based_on_track_occupancy()
    sections.update_track_power_section_switches()
    interlocking.process_interlocking_east()
    interlocking.process_interlocking_west()
    schematic.update_track_schematic(canvas)
    schematic.refresh_signal_aspects()
