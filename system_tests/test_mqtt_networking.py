...
def run_remote_track_sensor_tests(delay:float=0.0):
    ...
    sleep(delay)
    sleep(network_delay)
    print("MQTT remote track sensor tests - set logging level to debug to see the DCC messages being sent")
    logging.getLogger().setLevel(logging.DEBUG)
    set_signals_off(5)
    sleep(delay)
    sleep(network_delay)
    logging.getLogger().setLevel(logging.WARNING)
    print("MQTT remote track sensor tests - set logging level back to WARNING")
